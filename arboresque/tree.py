...
class DecisionTreeClassifier:
    def __init__(self, criterion="gini", max_depth=None, min_samples_split=2,
        ...
        """
        yet to handle missing values
        yet to add weights for classes and samples
        """
        ...
        for ft in self.cat_fts:
            ...
            if num_cats>20:
                msg = (f"The feature at index {ft} has {num_cats} categories. "
                       ...
                       f"features with at most twenty categories as categorical features.")
                warnings.warn(msg)
                continue
    ...
    def fit(self, X, y):
        ...
        if isinstance(self.min_samples_split, float):
            self.min_samples_split = max(2,np.ceil(self.min_samples_split * n_samples))

        if isinstance(self.min_samples_leaf, float):
            self.min_samples_leaf = max(2, np.ceil(self.min_samples_leaf * n_samples))
